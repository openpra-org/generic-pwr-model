...
def parse_and_dump_json(json_input_directory, json_output_directory):
...
def parse_and_dump_xml(xml_input_directory, xml_output_directory):
...
def convert_json_to_xml(json_input_directory, xml_output_directory):
    # List all JSON files in the JSON input directory
    json_files_to_be_converted = [file for file in os.listdir(json_input_directory) if file.endswith('.JSInp')]

    # Parse each JSON file and convert it to XML
    for file_name in json_files_to_be_converted:
        input_file_path = os.path.join(json_input_directory, file_name)
        output_file_path = os.path.join(xml_output_directory, f'converted_{os.path.splitext(file_name)[0]}.xml')
        ...
        try:
            ...
            print(f"Error in JSON file {file_name}: {e}. Skipping parsing and converting.")


def main():
    # # Directories for JSON parsing and dumping
    ...
    #
    # # Directories for XML parsing and dumping
    ...
    # Convert JSON to XML
    json_to_xml_input_directory = '/Users/afshar-flow/Repo/Gitlab/Enhancement-of-PRA-Tools/Model-Exchange/model-converter/saphsolve_actual_models/generic_PWR/'
    xml_converted_output_directory = '/Users/afshar-flow/Repo/Gitlab/Enhancement-of-PRA-Tools/Model-Exchange/model-converter/saphsolve_to_openpsamef_actual_models/generic_PWR'

    # # Perform JSON parsing and dumping
    # parse_and_dump_json(json_input_directory, json_output_directory)
    #
    # # Perform XML parsing and dumping
    # parse_and_dump_xml(xml_input_directory, xml_output_directory)

    # Convert JSON to XML
    convert_json_to_xml(json_to_xml_input_directory, xml_converted_output_directory)
